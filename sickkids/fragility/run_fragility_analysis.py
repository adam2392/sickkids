...
def run_row_analysis(state_deriv_fpath, radius, deriv_path,
                     plot_heatmap=True, figures_path=None):
    state_deriv = read_derivative_npy(state_deriv_fpath)

    pert_deriv, deltavecs_deriv = state_perturbation_derivative(state_deriv, radius=radius,
                                                                perturb_type='R')
    # save output
...
def run_analysis(
        bids_path, reference="monopolar", resample_sfreq=None, deriv_path=None, figures_path=None,
        verbose=True, overwrite=False, plot_heatmap=True, plot_raw=True, **model_params
):
    ...
    # load in raw data
    raw = load_data(bids_path, resample_sfreq, raw_figures_path, plot_raw=plot_raw, verbose=verbose)
    ...
    perturb_deriv, state_arr_deriv, delta_vecs_arr_deriv = lds_raw_fragility(
        raw, order=order, reference=reference, return_all=True, **model_params,
    ...
    if plot_heatmap:
        ...
        print(f'Resected channels are {resected_chs}')
        print(f"saving figure to {figures_path} {fig_basename}")
        perturb_deriv.plot_heatmap(soz_chs=resected_chs,
            cbarlabel="Fragility",
            cmap="turbo",
    ...
    run_row_analysis(state_deriv_fpath, model_params['radius'], deriv_path,
                     plot_heatmap=True, figures_path=figures_path)
