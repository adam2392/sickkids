...
def setup_simulation(
    ...
    **epileptor_kwargs
    ...
    r = np.array(
        epileptor_kwargs.get("r", 0.00035 / 7)
    )  # Temporal scaling in the third state variable
    Ks = np.array(
        epileptor_kwargs.get("Ks", -0.5)
    )  # Slow Permittivity coupling, fast to slow time scale
    Kf = np.array(
        epileptor_kwargs.get("Kf", 0.1)
    )  # Permittivity coupling of spike wave events
    Kvf = np.array(
        epileptor_kwargs.get("Kvf", 0.0)
    )  # Permittivity coupling of spike wave events
    tt = np.array(epileptor_kwargs.get("tt", 0.10))  # time scale of simulation
    tau = np.array(
        epileptor_kwargs.get("tau", 10)
    )  # Temporal scaling coefficient in fifth st var
    ...
    if noise_cov is None:
        noise_cov = np.array([0.001, 0.001, 0.0, 0.0001, 0.0001, 0.0])
        if resting_state:
            noise_cov = np.append(noise_cov, [0.0001, 0.0])
