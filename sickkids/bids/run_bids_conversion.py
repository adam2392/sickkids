...
import numpy as np
import pandas as pd
from eztrack.preprocess.bids_conversion import append_original_fname_to_scans
...
from sickkids.bids.tvb_dataset import _set_ch_types_tvb
from sickkids.bids.utils import (_check_bids_parameters, _look_for_bad_channels, _channel_text_scrub)
...
def write_edf_to_bids(edf_fpath: [str, Path],
                      ...
                      line_freq: int = 60, montage=None) -> Dict:
    ...
    # ch_mapping = _set_ch_types(raw, entities.get('subject'),
    ...
    channels_tsv_fname = _find_matching_sidecar(output_bids_path, suffix='channels',
                                                extension='.tsv', on_error='ignore')
    print(f'The channels tsv file is... {channels_tsv_fname}')
    electrodes_tsv_fname = _find_matching_sidecar(output_bids_path, suffix='electrodes',
                                                  extension='.tsv', on_error='ignore')
    ...
    #         _update_sidecar_tsv_byname(channels_tsv_fname, ch,
    #                                    'group', group)
    #         _update_sidecar_tsv_byname(electrodes_tsv_fname, ch,
    #                                    'group', group)

    # update reference channel
    # reference_ch = ch_mapping['reference']
    # for ch in [raw.ch_names[idx] for idx in mne.pick_types(raw.info, meg=False, ecog=True,
    #                                                        seeg=True, exclude=[])]:
    #     _update_sidecar_tsv_byname(channels_tsv_fname, ch,
...
def convert_tvb_dataset():
    ...
    from sickkids.bids.tvb_dataset import add_data_to_participants
    ...
    for subject in subject_ids:
        ...
        df = pd.read_csv(ch_xyz_fpath, delim_whitespace=True,
        ...
        for run_id, fpath in enumerate(source_paths):
            ...
            output_dict = write_edf_to_bids(edf_fpath=fpath,
                                            ...
                                            bids_root=root, line_freq=line_freq,
                                            montage=montage)
            ...
            electrodes_tsv_fpath = BIDSPath(
                ...
                space=space, acquisition=modality, datatype=datatype,
            )
        ...
        for subject in subject_ids:
            ...
            for task, filepaths in task_filepaths.items():
                for run_id, fpath in enumerate(filepaths):
                    ...
                    output_dict = write_edf_to_bids(edf_fpath=fpath,
                                                    bids_kwargs=bids_kwargs,
                                                    bids_root=bids_root, line_freq=line_freq)
                    bids_fname = output_dict['output_fname']
