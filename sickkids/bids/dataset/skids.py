...
def _set_ch_types(raw, subject, session):
    ...
    if subject == 'E1':
        ...
        if session in ['postresection', 'intraresection']:
            disconnected_chs.extend(['C15', 'C17', 'C18', 'C19', 'C20'] + \
        ...
        disconnected_chs = []
        if session in ['postresection', 'intraresection']:
            disconnected_chs.extend([f'C{idx}' for idx in range(1, 6)] + \
        ...
        disconnected_chs = []
        if session in ['postresection', 'intraresection']:
            disconnected_chs.extend(['C2', 'C7', 'C16'] + [f'C{idx}' for idx in range(10, 15)] + \
        ...
        disconnected_chs = []
        if session in ['postresection', 'intraresection']:
            disconnected_chs.extend(['C15', 'C49'] + [f'C{idx}' for idx in range(17, 21)] + \
        ...
        grids = [f'C{idx}' for idx in range(1, 65)]
        strips = [f'ATPS{idx}' for idx in range(1, 7)] + \
        ...
        disconnected_chs = []
        if session in ['postresection', 'intraresection']:
            disconnected_chs.extend(['C12', 'C24', 'C38', 'C41', 'C42', 'C49', 'C50', 'C51'] + \
...
def add_data_to_participants(subject, bids_root):
    ...
    for key, value in entries:
        ...
        if description == '':
            description = None
        update_participants_info(bids_root, subject, key, value,
